...
% Define and calculate some variables and values before we start
    ...
    %Calc the gradient of the running median
        ...
        %Set all grad trace before trigger to zero (don't see events
        %before trigger).
    if strcmp(Options.NoFusionAllowedBeforeTrigger,'y')
...
% negative)
    ...
    if strcmp(Options.TypeofFusionData, 'TetheredVesicle')
        ...
        elseif strcmp(Options.FusionTrigger, 'pH')
            VirusStopbyIndex = find(TraceRunMedian.FrameNumbers==...
                max(UniversalData.pHDropFrameNumber + 1,Options.FrameToStartAnalysis + Options.RunMedHalfLength));
        end
        
            % This determines where we will start analyzing for fusion events
            if isempty(VirusStopbyIndex)
                disp("Error: couldn't find the start frame in the cropped data (you probably chose to ignore the frame right after the pH drop, which isn't allowed.)")
...
% -------------------- Spike Test --------------------
    ...
    % intensity value, or something bright that flies by quickly out of focus
        ...
        SpikeTrace = TraceRunMedian.Trace(StartIdx:EndIdx) - TraceRunMedianWiderRange; 
