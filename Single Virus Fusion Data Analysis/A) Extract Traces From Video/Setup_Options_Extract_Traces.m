...
% ==============Options you'll probably change regularly=================
    ...
    Options.ScanRange = [30,60];
        ...
        % Usually you will use this to scan the threshold that you will
        % use for finding. If so, choose 'y' and the program will scan
        % between the range you choose and then terminate.
        
    Options.Threshold = 53;
    ...
    Options.ExtraLabel =  strcat('_ExtractedTraces');
...
%================= Options You Might Change While Optimizing For a Particular Data Type (after consulting Bob) ================
    ...
    Options.DeterminePixelOffset = 'n';
    ...
    % ---------Inputs specific to each data set---------
        ...
        Options.TimeInterval = 0.3; %in sec
        Options.TimeVector = 0:Options.TimeInterval:300-Options.TimeInterval;
            % Input time vector used for all data. This will be
            % ignored if times extracted from metadata.
        
        Options.ExtractInputsFromTextFile = 'y';
            % 'y' OR 'n'
            ...
            % FocusFrameNumbers, PHdropFrameNum, etc. In that case, the numbers put in for those 
        ...
        % external .txt file, which is ununusual. So usually ignore these.
            Options.BindingTime = 0; %in sec, doesn't have to be integer
            ...
            Options.FrameNumToFindParticles = 32;
                % This is the frame number that will be used as the finding image, 
                % to find viral particles. If you want to choose a different file
                % to find the particles then input Options.FrameNumToFindParticles = "Other"; 
                % The program will then ask you to choose the finding image after
                % it asks you to choose the video to be analyzed.
                % Note that the "Other" option will not work if you are extracting
                % times from meta data.
                % Note: If you chose "Other" option, the program will ignore the
                % FindFramesToAverage below, and instead will average every image
                % in the "Other" file.
                % NOTE: This number will be ignored if you choose to extract inputs from associated text file.
        ...
        %------Inputs that would be unusual to change for DENV analysis------
            Options.ExtractTimesFromMetaData = 'n';
            ...
            Options.DetermineBindingTimeFromTimestamp = 'n';
            ...
            if strcmp(Options.DetermineBindingTimeFromTimestamp, 'y')
                ...
                % This is the frame number that will be used to determine time
                % zero. Default is 1. You likely shouldn't change this number.
                % NOTE: This number will be ignored if you choose to extract inputs from associated text file.
...
% ===========Options you are unlikely to ever change===============
    ...
    % --------- Combine Multiple Videos Options---------
        Options.CombineMultipleVideos = 'n';
         ...
         Options.NumberOfVideosToCombine = 2;
