...
if strcmp(Options.FrameNumToFindParticles,"Other")
    FindVideoFilePath = Options.FindImageFilePath;
    FindImageInfo = imfinfo(FindVideoFilePath);
    FrameRange = 1:length(FindImageInfo);
else
    ...
    FindVideoFilePath = VideoFilePath;
    FindImageInfo = VideoInfo;
end

ImageWidth = FindImageInfo.Width; %in pixels
ImageHeight = FindImageInfo.Height; %in pixels
BitDepth = FindImageInfo.BitDepth;
...
if strcmp(Options.ExtractTimesFromMetaData, 'y')
    ...
    FrameCounter = 0;
    for b = FrameRange
        CurrentFrameImage = imread(FindVideoFilePath,b);
