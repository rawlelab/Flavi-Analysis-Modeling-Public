...
% to match the way you format your data.
    ...
    Datalabel=  DefaultPathname;
            ...
            DataFileLabel = Datalabel(IndexofSlash(end-1)+1:end-1);
                %File label will be the name of the parent folder in which
                %the data came from
            % FileInfo = Datalabel(IndexofSlash(end-1):end);
            % IndextoStop = find(FileInfo == '-');
            
%             if numel(IndextoStop)==0
%                 DataFileLabel = 'NoLabel';
%             else
%                 DataFileLabel = FileInfo(2:IndextoStop(1)-1);
%             end
